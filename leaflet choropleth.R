...
# max min for scale
max(obese_overweight$Percentage)
min(obese_overweight$Percentage)
...
z <- leaflet() %>%
  ...
  addPolygons(data = usa_states,fillOpacity = 0.65,fillColor = palette(obese_overweight$mean),color="white", weight = 1)
z
